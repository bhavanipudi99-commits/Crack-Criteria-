...
start_marker = "  const renderCanvasComposer = () => {"
end_marker = "  // RENDER: CRITERIA TABLE BUILDER"
    ...
    return (
      <div className="h-full flex flex-col bg-slate-50 relative">
        ...
        <div className="flex-1 flex flex-col overflow-hidden relative">
          ...
          <div className="h-64 border-b border-slate-200 bg-slate-50 p-4 flex flex-col flex-shrink-0 shadow-inner z-10">
                ...
                {type === 'CANVAS' && Array.from({ length: Math.max(max, totalSelectedTiles) }).map((_, i) => {
                  ...
                  return (
                    <div key={i} onClick={() => tile && toggleTileInCanvas(tile.id, canvas.id, activeQ.id)}
                      className={`min-h-[3rem] p-2 rounded-xl border-2 flex flex-col items-start justify-between cursor-pointer transition-all ${tile ? 'bg-blue-600 border-blue-500 text-white shadow-md hover:bg-rose-600 hover:border-rose-500 hover:scale-[1.02]' : 'bg-slate-800/50 border-slate-700 border-dashed text-slate-500 hover:bg-slate-800 hover:border-slate-600'}`}>
                      {tile ? (
                        <>
